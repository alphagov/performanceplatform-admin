...
class AboutYourServicePageTestCase(FlaskAppTestCase):
    ...
    def test_service_name_field_is_required(self):
        data = self.params({'service_name': ''})
        response = self.app.post('/register/about-your-service', data=data)
        assert_that(response.status, equal_to('200 OK'))
        assert_that(response.data, contains_string('Name cannot be blank'))

    def test_service_url_field_is_optional(self):
        data = self.params()
        response = self.app.post('/register/about-your-service', data=data)
        assert_that(response.status, equal_to('302 FOUND'))

    def test_service_url_is_like_a_url(self):
    ...
    def test_service_description_field_is_required(self):
    ...
    def test_stores_submitted_data_in_the_session(self):
    ...
    def test_redirects_to_confirmation_page(self):
        data = self.params()
        response = self.app.post('/register/about-your-service', data=data)
        assert_that(response.status, equal_to('302 FOUND'))
        assert_that(response.headers['Location'],
